...
public abstract class RecyclerViewDragHolder {
    ...
    private class DragLinearLayout extends FrameLayout {
        ...
        private class ViewDragCallBack extends ViewDragHelper.Callback {
            ...
            /**
             * 手指释放后回调，但只能拖动之后才有回调？因为实验后发现只是点击的话没有这个回调
             * @param releasedChild
             * @param xvel
             * @param yvel
             */
            ...
            public void onViewReleased(View releasedChild, float xvel, float yvel) {
                ...
                if (mTrackingEdges == EDGE_LEFT) {
                    if (topView.getLeft() < (int) ((float) bgWidth * scrollPercent) || state == STATE_OPEN) { // 不应该是&&底层是打开的么？
                        newLeft = viewX;
                        state = STATE_CLOSE;
                    } else { // 打开底层view
                        newLeft = bgWidth;
                    ...
                    if (topView.getLeft() > -(int) ((float) bgWidth * scrollPercent) || state == STATE_OPEN) {
                        newLeft = viewX;
                        state = STATE_CLOSE;
                    } else { // 打开底层view
                        newLeft = -1 * bgWidth;
                ...
                // 如果smoothSlideViewTo返回true，继续调用continueSettling(boolean)
             ...
             * @param left 移动达到的x轴的距离
             * @param dx 建议的移动的x距离
             * @return
             */
            @Override
            public int clampViewPositionHorizontal(View child, int left, int dx) {
                Log.e("clampViewPositionHori", "left = " + left + "\tdx = " + dx);
                if (mTrackingEdges == EDGE_LEFT) {
            ...
            public int getViewHorizontalDragRange(View child) {
                Log.e("getViewHorizontalDragRa", "getViewHorizontalDragRange");
                return topView == child ? child.getWidth() : 0;
            }

            @Override
            public int getViewVerticalDragRange(View child) {
                Log.e("getViewVerticalDragRa", "getViewVerticalDragRange");
